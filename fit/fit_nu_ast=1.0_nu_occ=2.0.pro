...
io_file='fit_nu_ast=1.0_nu_occ=2.0'
...
if (keyword_set(mosaic) eq 0) then begin $
  ;rebin mosaic 
  grid_factor=5 & $
  restore, filename=  '../mosaic/mercator.dat'&$
  ...
  for i=0,Nx-1 do begin $
    for j=0,Ny-1 do begin $
      ...
      if (Nk gt 2*grid_factor^2) then begin $
        ...
        good_fix(i,j)=1 & $
        mom=moment(sub_mosaic(k)) & $
        err(i,j)=sqrt(mom[1]/(Nk-1)) & $
      endif & $
    endfor & $
  endfor & $
...
print,'Brightest contour = ',lvls(n_elements(lvls)-1)*cal, $
  ' solar brightness units'

;plot ecliptic surface brightness profile
window,xs=550,ys=550
phi=phi_grid(Ngrid/2:*)
profile=model(Ngrid/2:*,Ngrid/2)
plot_oo,phi*r2d,profile,xrange=[1,30],xstyle=1
j=where(phi ge 10.0/r2d)
j=j(0)
oplot,phi*r2d,profile(j)*(sin(phi(j))/sin(phi))^2.45,linestyle=2
