...
if (keyword_set(mosaic) eq 0) then begin $
  ...
  x_sun=Nx/2 & $
  y_sun=Ny/2 & $
  x=ps_avg*grid_factor*(findgen(Nx)-x_sun) & $
  y=ps_avg*grid_factor*(findgen(Ny)-y_sun) & $
...
if (calc_SB ne 0) then begin $
  ...
  save,filename=io_file,SB,sin_epsilon & $
endif

stop

;Calculate dust model in units of counts/sec/pixel
restore,filename=io_file
...
for j=0,Nparams-1 do begin $
  i=where(chi2(*,j,kk) eq min(chi2(*,j,kk))) & $
  ast_range(j)=ast(i) & $
endfor
