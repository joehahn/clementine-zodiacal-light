...
zap_mask(0:110,*)=0		;zap left edge
zap_mask(287:291,*)=0		;zap columns near Venus
zap=where(zap_mask eq 0)

;set fix_venus=1 to replace pixels polluted by Venus with a
;local average. Set=0 to skip this step.
fix_venus=1
...
if (fix_venus eq 1) then begin $
  xV=289 & $				;Venus x-coordinate
  yV=421 & $				;Venus y-coordinate
  rV=18	 & $				;Venus radius
  ...
  for t=0,Nimages-1 do begin $
    ;im=images(*,*,t) & $
    ;im=im-median(im(x_dark0:x_dark1,y_dark0:y_dark1)) & $
    ;tvscl,alog(im*max(exp_time)/exp_time(t)>10) & $
    ...
    ;im=images(*,*,t) & $
    ;im=im-median(im(x_dark0:x_dark1,y_dark0:y_dark1)) & $
    ;tvscl,alog(im*max(exp_time)/exp_time(t)>10),Nx,0 & $
  endfor & $
