%
%       latex clementine
%       dvips -Pcmz -Pamz -o clementine.ps clementine
%       distill clementine.ps
%
...
\begin{center}
Review copy submitted to {\it Icarus}.\\
Do not disseminate further.\\
September 27, 2001\vspace*{8ex}
\end{center}
...
interplanetary dust also has an effective albedo of $a=0.17$
...
are the source of about $45\%$ of the dust cross--section seen
...
uncertain by a factor of $\sim500$). Since the flow of
interstellar gas and dust ultimately strips these wide--ranging
Oort Cloud dust grains from the solar system, these finding
suggests that the Sun and perhaps other stars may have vast
...
observation dates, ecliptic longitudes, and total exposure times
for the subset of the data that are examined here. The camera's
...
Figure \ref{raw}C shows a text--book quality image of the
...
elongation of the line of sight having an ecliptic latitude
...
$\psi$. An empirical phase law that reproduces a suite of
zodiacal light measurements acquired
over a wide range of elongation angles is generated using the
recipe given in \citet{Hong85} (see Fig.\ \ref{phase_fn}).
Although this phase law was formally constructed to fit
observations acquired at elongations of
$15\le\epsilon\le180^\circ$, we have verified that it is indeed
consistent with these data that were acquired at
smaller phase angles. Figure \ref{phase_fn} also plots the
integrand of Eq.\ (\ref{Z}) which shows that the dominant
contribution to the surface brightness integral is due to dust
in the vicinity of $\varphi\simeq90^\circ$, {\it i.e.}, dust
nearer the Sun along the line--of--sight. For dust observed in
the ecliptic at elongations $\epsilon\le30^\circ$, we find that
about $90\%$ of the light is reflected by dust orbiting interior
to $0.6$ AU. Note also that the phase
function is zero at $\varphi\le23^\circ$, indicating that there
is no forward--scattering of light by the dust that would
very lie near the observer. This particular behavior means that
...
For this case the integral evaluates to $\simeq0.826$
and Eq.\ (\ref{Z}) simplifies to
\begin{equation}
\label{Zo}
Z(\epsilon)\simeq1.79\times10^{-5}
\frac{a\sigma_1 r_1}{\sin^{\nu+1}\epsilon}B_\odot.
\end{equation}
Thus the radial power law $\nu$ is readily obtained from
...
$Z(\epsilon)=(1.7\pm0.2)\times10^{-13}B_\odot/
(\sin\epsilon)^{(2.45\pm0.05)}$, so $\nu=1.45\pm0.05$.
These uncertainties include both
the statistical variations of the data in
Fig.\ \ref{profiles} as well as the $8\%$ uncertainty 
in the calibration. Note that the radial
$\nu$ reported here is slightly steeper than that inferred from
the data obtained by the Helios spacecraft \citep{Letal81},
with the disagreement at the $2\sigma$ level.

Note the north--south and east--west asymmetries seen in
Fig.\ \ref{profiles}. Ratios of the north/south and east/west
surface brightness profiles are also plotted versus in
Fig.\ \ref{ratios}. This Figure shows that the northern
hemisphere get steadily brighter with elongation
...
previously observed by the Helios 1 and 2 spacecraft which
detected a $i=3^\circ$ tilt between the midplane of the inner
zodiacal light and the ecliptic \citep{LHRP80}. The node
of this symmetry plane has a
longitude of $\Omega=87^\circ$ which, as Fig.\ \ref{longitudes}
shows, is largely perpendicular to the Clementine
lines--of--sight, and this particular viewing geometry tends to
make the northern hemisphere slightly brighter than the southern.
Gravitational perturbations by giant planets
can also displace the zodiacal cloud's center of light
radially away from the Sun, a phenomenon also known as
pericenter glow \citep{Wetal99}, and these
perturbations may be responsible for the east--west
asymmetry seen in Figs.\ \ref{profiles}--\ref{ratios}.
...
\begin{array}{ll}
1 & {j=\mbox{iso}}\\
c_je^{-(i/\sigma_j)^2/2} & \mbox{othewise}
\end{array}
\right.
\end{equation}
...
`effective' albedo of the dust ensemble. However the albedos of
asteroidal and cometary dust are not known with any
certainty so we will simply set $a_j/a=1$.
...
and interstellar dust sources. Figure \ref{asteroid_inc}
shows the inclination distribution for near--Earth, main--belt,
...
are shown in Fig.\ \ref{comet_inc} where they are divided into
...
The smooth curves in Fig.\ \ref{comet_inc} also show
...
\ref{comet_inc} suggests that selection effects may be
...
latitude-longitude grid using Eq.\ \ref{Z_j}, and isophotes 
for example populations are shown in Fig.\ \ref{isophotes}. A
...
these dust grains can also be deposited {\it in situ}.
Consequently we expect the three populations to have
$1\lesssim\nu_j\lesssim2$ or so.
...
yet been considered (see Fig.\ \ref{asteroid_inc}); these
...
(e.g., HTCs and OCCs). 
The inferred inclination distributions for all of the dust
populations are shown in Fig.\ \ref{inclination}. Note that these
distributions are rather similar to that reported
...
the spatial density of interplanetary dust cross section at 1 AU
to be $\sigma_1\sim4.6\times10^{-21}$ cm$^2$/cm$^3$.
Since Table \ref{best_fit} reports
$a\sigma_1 r_1=(1.2\pm1.0)\times10^{-8}$, this implies that the
dust have an effective albedo of $a\simeq0.17$. Note, however,
that this estimate for $\sigma_1$ and
...
If our findings can be extrapolated
out to this distance, the Eq.\ (\ref{Sigma0}) predicts a total
surface area $\Sigma_{low}(r_3)=6.8\times10^9$ km$^2$ for the
low--inclination  dust component. Note that observations of the
...
12 km across. Note that this mass limit is about 3.5 times
the mass of the asteroidal dust bands
detected by the Cosmic Background Explorer (COBE)
\citep{Retal97}, but keep in mind that our
...
Ultimately these wide--ranging Oort Cloud dust grains will
be stripped from the solar system by interstellar gas and dust.
The larger grains will fragment upon colliding with interstellar
dust, and the first collision occurs after traveling a
distance $r_c\sim1/n_\star\pi R_c^2$. 
The circumference of the grain's very eccentric orbit is
$\sim4a$, so the collision timescale $t_c$ in units of the
grain's orbital period $P_{orb}$ is
\begin{equation}
\label{t_c}
\frac{t_c}{P_{orb}}\sim\frac{r_c}{4a}\sim
\frac{1}{4\pi n_\star R_c^2a}\sim
0.9\left(\frac{R_c}{100\ \mu\mbox{m}}\right)^{-2}
\left(\frac{a}{10^4\mbox{ AU}}\right)^{-1}.
\end{equation}
Thus grains larger than $R_c\sim100\ \mu$m do not persist longer
than about one orbit.

However, smaller dust grains are more easily entrained by the
interstellar gas flow. A grain's specific orbital energy is
$E=-GM_\odot/2a$ where $G$ is the gravitation constant and
$M_\odot$ the solar mass, so a gradual velocity kick of
$\Delta v\sim\sqrt{2|E|}=\sqrt{GM_\odot/a}\sim0.3$ km/sec
will remove the grain from the solar system. This occurs
when the dust grain sweeps out a gas volume having a momentum
content comparable to the grain's mass$\times\Delta v$, and this
happens after traveling a distance
$r_s\sim(4\rho R_c\sqrt{GM_\odot/a})/3\rho_gv_\infty$
where $\rho_g$ is the density of interstellar gas. For a
gas density of $\rho_g=5.6\times10^{-25}$ gm/cm$^3$
\citep{Frisch00}, the dust--stripping timescale $t_s$ is
\begin{equation}
\label{t_s}
\frac{t_s}{P_{orb}}\sim\frac{r_s}{4a}\sim5
\left(\frac{R_c}{1\ \mu\mbox{m}}\right)
\left(\frac{\rho}{1\ \mbox{gm/cm}^3}\right)
\left(\frac{a}{10^4\ \mbox{AU}}\right)^{-3/2}
\end{equation}
where $P_{orb}=10^6(a/10^4\mbox{ AU})^{3/2}$ years. Equations
(\ref{t_c}--\ref{t_s}) thus indicate that Oort Cloud comet
dust is removed on timescales of $\sim1$--20 orbits. This also
suggests that the solar system itself may have a vast but
tenuous cometary dust tail. If so, then the width of this
tail will be of order $\sim10^4$ AU and will be
oriented in the downstream direction of the local interstellar
flow which has a longitude, latitude
of $(74.7^\circ, -4.6^\circ)$ in ecliptic coordinates
\citep{Frisch00} or a right ascension, declination
of $(73.9^\circ, +18.0^\circ)$ in equatorial coordinates.
Other stars might also have Oort Comet Clouds and thus have
stellar dust tails, which, if detectable, would act as banners
that reveal the direction of local interstellar flows.
Also recall that an Oort Cloud forms when a
system of recently--formed  giant planets scatters the residual
planet--building debris out into wide orbits. Thus the discovery
of a stellar dust tail might also serve as an indicator of
giant planets orbiting within.
...
$a\sigma_1r_1=(1.2\pm0.1)\times10^{-8}$. Assuming that the
...
Next, get the star's ecliptic longitude and latitude
...
\hline
     & $a\sigma_1r_1=(1.2\pm0.1)\times10^{-8}$
\enddata
\end{deluxetable}
...
\caption{The narrow lines show the longitudes of the spacecraft's
...
and $\gamma$ the direction of the vernal equinox.
Longitudes are measured counter--clockwise from $\gamma$.
The dashed circle represents the tilt of the zodiacal light's
midplane as reported by \citet{LHRP80} and having an
inclination $i=3^\circ$ and a longitude of ascending node
$\Omega=87^\circ$. The galactic north pole GNP is also
indicated, and it has an ecliptic longitude
...
This linear greyscale is saturated at 20\% of the pixels'
capacity in order to reveal the faint artifacts present in the
...
adjacent pixels. {\bf B}.\ A logarithmic greyscale of the
master image for orbit 193.
...
observed by the Clementine star tracker camera. The colors
correspond to logarithmic changes in the surface brightness.
...
volume element a distance $r$ from the Sun at latitude $\beta$
measured from the ecliptic $x$--$y$ plane. The observer has a
heliocentric distance $r_1$ and the line--of--sight (LOS) to
the volume element a distance $\Delta$ away has a latitude
...
\figcaption{The solid curve is an empirical phase law $\psi$
plotted versus scattering angle $\varphi$ obtained
using the method of \citet{Hong85} and drawn for
a radial power--law exponent $\nu=1.45$. This phase law is
normalized to unity at $\varphi=\pi$ where the
phase angle $\pi-\varphi=0$. Dust close to the observer have
a $\varphi$ close to $0^\circ$ whereas distant dust have
$\varphi$ near $180^\circ$. The dashed curve is the
integrand of Eq.\ (\ref{Z}) as a function of $\varphi$.
\label{phase_fn}}
\end{figure}
...
\begin{figure}
\epsscale{1.0}
\plotone{figs/asteroid_i_dist.ps}
\figcaption{The asteroid inclination distribution $N(i)$
...
These data come from {\it The Asteroid Orbital Elements
Database} that is maintained by Edward Bowell, and they are
obtained at the URL ftp://ftp.lowell.edu/pub/elgb/astorb.html.
The smooth curve is an inclination distribution having the form
$g(i)\propto\sin(i)e^{-(i/\sigma)^2/2}$
where $\sigma=6.2^\circ$.
\label{asteroid_inc}}
\end{figure}

\begin{figure}
\epsscale{1.0}
\plotone{figs/comet_i_dist.ps}
\end{figure}

\begin{figure}
\figcaption{The inclination distributions for 138
...
$g(i)\propto\sin(i)e^{-(i/\sigma)^2/2}$
with $\sigma=8.0^\circ$ for the JFCs, $\sigma=33^\circ$ for the
...
from OCCs and interstellar sources). The dust
cross--sectional density varies as $\propto r^{-\nu_j}$
with $\nu_j=1.45$ for each population, and the brightness of each
...
offsets $\delta f$ that are removed from each image (see Section
...
plotted versus latitude $\beta$. Also shown are the
weighted contributions by the low, high, and isotropic
populations, $f_jh_j(\beta)$.
\label{latitude}}
\end{figure}
