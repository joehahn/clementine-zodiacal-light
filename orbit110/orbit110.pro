...
for t=0,Nimages-1 do begin $
  ...
  images(0,0,t)=im & $
  ;tvscl,(im+3.0*template)*max(exp_time)/exp_time(t)>(-2)<8 & $
...
window,xs=Nx,ys=Ny
tvscl,im
contour,alog(smooth(avg_image,6)>1),nlevels=7,xstyle=1, $
...
contour,alog(smooth(avg_image,6)>1),nlevels=8,xstyle=1, $
  title='Orbit '+strtrim(string(orbit),2),charsize=1.3, $
  xtitle='pixels',ytitle='pixels',ystyle=1
