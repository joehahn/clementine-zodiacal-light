...
lead_name='/u2/hahn/clementine/new253/conversions/lba'
...
alpha1	=  0.554898
delta1	=  0.409498
x2	=  262.1		;Ari rho 3-46 & 2-45
y2	=  28.3			;not used forRA-DEC solution as it
alpha2	=  0.769843		;gives a poor solution.
delta2	=  0.314562
x3	=  151.3		;Ari Gamma 2-5   
y3	=  183.4
alpha3	=  0.495368
delta3	=  0.336737
x4	=  177.9		;Ari 15   
...
zap_mask(275:279,*)=0		;zap columns near Venus
...
if (fix_venus eq 1) then begin $
  xV=277 & $				;Venus x-coordinate
  yV=57 & $				;Venus y-coordinate
  rV=19	 & $				;Venus radius
...
for t=0,Nimages-1 do begin $
  ...
  images(0,0,t)=im & $
  ;tvscl,(im+3.0*template)*max(exp_time)/exp_time(t)>0<40 & $
...
register_image,images,23,dx0,dy0,MAG=5,X0=x0-w,X1=x0+w,Y0=y0-w,Y1=y0+w
;using star1
register_image,images,23,dx1,dy1,MAG=5,X0=x1-w,X1=x1+w,Y0=y1-w,Y1=y1+w
;using star2
register_image,images,23,dx2,dy2,MAG=5,X0=x2-w,X1=x2+w,Y0=y2-w,Y1=y2+w
;plot the 3 different offsets for dx & dy, average them,
;and then round off to do full-pixel shifts.
!p.multi=[0,1,2,0,0]
plot,dx0,psym=8,yrange=[-8,4],xrange=[-1,Nimages+1],xstyle=1, $
  ystyle=1,xtitle='image index',ytitle='!7D!3x    (pixels)'
oplot,dx1,psym=6
oplot,dx2,psym=1,symsize=1.5	;don't use star2---too faint!
dx=round((dx0+dx1)/2.0)
...
dy=round((dy0+dy1)/2.0)
oplot,dy,thick=3,psym=10
oplot,0.5*(dy0+dy1),psym=10
!p.multi=0
...
y_opt=286.0
x_star=[ x0, x1, x3, x4, x5, x6]	
y_star=[ y0, y1, y3, y4, y5, y6]
RA_star = [ alpha0, alpha1, alpha3, alpha4, alpha5, alpha6]
DEC_star= [ delta0, delta1, delta3, delta4, delta5, delta6]
.run plate_scale
...
;get RA & DEC---don't worry about non-convergence
...
contour,alog(avg_image>1<300),xrange=x6+w*[-1,1], $
