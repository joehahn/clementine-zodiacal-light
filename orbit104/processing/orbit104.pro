...
lead_name = '/home/jhahn/science/clementine/orbit104/data/lba'
...
for t=0,Nimages-1 do begin $
  ...
  lbl_file=strmid(lbl_file, 0, length-4)+'lbl' & $
  ...
  tvscl,im*max(exp_time)/exp_time(t)>0<250 & $
  wait, 0.05 & $
...
j=where((mask eq 1) and (images gt 160), Nj)
if (Nj gt 1) then plot,images(j),psym=3
if (Nj eq 0) then print, 'all images are saturated'
...
;find sun
elongation = sqrt(phi^2 + theta^2)
mn = min(elongation, sun_pix)
sun_pix_xy = array_indices(elongation, sun_pix)
x_sun = sun_pix_xy[0]
y_sun = sun_pix_xy[1]

;view images, with sun located
window, xs=2*Nx, ys=Ny, retain=2
for t=0,Nimages-1 do begin $
  im=images(*,*,t) & $
  factor=max(exp_time)/exp_time(t) & $
  im = im*factor>0<100 & $
  im[x_sun, y_sun] = 100 & $
  tvscl,im & $
  tvscl,im*mask(*,*,t), Nx, 0 & $
  wait, 0.2 & $
endfor

;save results
save, filename='orbit253.dat', images, mask, exp_time, plate_scl, target_distance, $
    theta, phi, x_sun, y_sun
...
pretty(258,0)=sub(58:64,*)>0<255
left=pretty(251:257,*)
right=pretty(265:271,*)
a=0.5*(left+right)
pretty(258,345)=a(*,345:*)
smth=smooth(pretty,2)
pretty(258,345)=smth(258:264,345:*)
radial,rad_sub,61,483,100,Ny
radial,rad_pretty,261,483,Nx,Ny
j_sub   =where(rad_sub    le 15.0)
j_pretty=where(rad_pretty le 15.0)
pretty(j_pretty)=sub(j_sub)<255.0
tvscl,enlarge(pretty(nx/2:*,ny/2:*),2)
...
if (fix_Venus eq 0) then begin $
  ...
  write_tiff,'pretty_picture.tif',rotate(transpose(pretty),2) & $
endif
...
;make tiff image
write_tiff,'reduction_ABC.tif',transpose(rotate(ABC,1))
