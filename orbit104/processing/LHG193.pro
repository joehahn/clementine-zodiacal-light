...
lead_name = '/home/jhahn/science/clementine/orbit193/data/lba'
...
trail_name='z.193.tif'

;flatfield & good-pixel filenames
flat_file='/home/jhahn/science/clementine/flatfield/flatfield_short_exp.fits'
good_file='/home/jhahn/science/clementine/flatfield/good_pix_short_exp.fits'
...
target_distance=fltarr(Nimages)             ;distance to center of Moon in km
altitude=fltarr(Nimages)                          ;altitude above the Moon in km
sc_x = fltarr(Nimages)                             ;SC's x coordinates wrt moon center, in km
sc_y = fltarr(Nimages)                             ;SC's y coordinates wrt moon center, in km
sc_z = fltarr(Nimages)                             ;SC's z coordinates wrt moon center, in km
zero='0'
...
for t=0,Nimages-1 do begin $
  ...
  print,file & $
  im=read_tiff(file) & $
  im = transpose(rotate(im,1)) & $    ;new tiff images are mirror reflection of the old data
  sz=size(im) & $
  Nx=sz(1) & $
  Ny=sz(2) & $
  if (t eq 0) then begin $
    ;some setup on first entry only
    window,xs=2*Nx,ys=Ny, retain=2 & $
    images=intarr(Nx,Ny,Nimages) & $
  endif & $
  images(0,0,t)=im & $

  ;read lbl file & read header info
  lbl_file=file & $
  length=strlen(lbl_file) & $
  lbl_file=strmid(lbl_file, 0, length-3)+'lbl' & $
  spawn, 'wc '+lbl_file, wc & $
  wc=strsplit(wc, ' ', /extract) & $
  Nlines=wc(0) & $
  ...
  j=where(strmid(header,0,17) eq 'EXPOSURE_DURATION') & $
  exp_time(t)=float(strmid(header(j(0)),19,10))/1000.0 & $
  ;print,'     exposure = ',exp_time(t),' sec' & $
  j=where(strmid(header,0,15) eq 'RIGHT_ASCENSION') & $
  RA(t)=float(strmid(header(j(0)),18,9))*!pi/180.0 & $
  j=where(strmid(header,0,11) eq 'DECLINATION') & $
  DEC(t)=float(strmid(header(j(0)),18,9))*!pi/180.0 & $
  j=where(strmid(header,0,16) eq 'RETICLE_POINT_RA') & $
  RA_ret(t,0)=float(strmid(header(j(0)),21,6))*!pi/180.0 & $
  RA_ret(t,1)=float(strmid(header(j(0)),28,7))*!pi/180.0 & $
  RA_ret(t,2)=float(strmid(header(j(0)),36,7))*!pi/180.0 & $
  RA_ret(t,3)=float(strmid(header(j(0)),44,7))*!pi/180.0 & $
  j=where(strmid(header,0,25) eq 'RETICLE_POINT_DECLINATION') & $
  DEC_ret(t,0)=float(strmid(header(j(0)),29,7))*!pi/180.0 & $
  DEC_ret(t,1)=float(strmid(header(j(0)),37,7))*!pi/180.0 & $
  DEC_ret(t,2)=float(strmid(header(j(0)),45,7))*!pi/180.0 & $
  DEC_ret(t,3)=float(strmid(header(j(0)),53,7))*!pi/180.0 & $
  j=where(strmid(header,0,11) eq 'TWIST_ANGLE') & $
  twist(t)=float(strmid(header(j(0)),18,8))*!pi/180.0 & $
  j=where(strmid(header,0,10) eq 'START_TIME') & $
  date(t)=strmid(header(j(0)),13,50) & $
  j=where(strmid(header,0,23) eq 'FOCAL_PLANE_TEMPERATURE') & $
  temp(t)=float(strmid(header(j(0)),25,10)) & $
  j=where(strmid(header,0,12) eq 'GAIN_MODE_ID') & $
  gain_id(t)=strmid(header(j(0)),20,5) & $
  j=where(strmid(header,0,14) eq 'OFFSET_MODE_ID') & $
  offset_id(t)=strmid(header(j(0)),20,5) & $
  j=where(strmid(header,0,15) eq 'INSTRUMENT_NAME') & $
  instrument(t)=strmid(header(j(0)),20,23) & $
  j=where(strmid(header,0,22) eq 'TARGET_CENTER_DISTANCE') & $
  target_distance(t)=float(strmid(header(j(0)),27,10)) & $
  j=where(strmid(header,0,19) eq 'SPACECRAFT_ALTITUDE') & $
  altitude(t)=float(strmid(header(j(0)),24,10)) & $
  ;print,'     altitude = ', altitude(t),' km' & $
  j=where(strmid(header,0,25) eq 'SC_TARGET_POSITION_VECTOR') & $
  sc_xyz = strmid(header(j(0)),29,38) & $
  sc_x(t) = float(strmid( sc_xyz, 0, 12)) & $
  sc_y(t) = float(strmid( sc_xyz, 13, 12)) & $
  sc_z(t) = float(strmid( sc_xyz, 26, 16)) & $
  print,  sc_xyz, ":", sc_x(t), sc_y(t), sc_z(t) & $
...
;register images and mask
; also shift all images left by 1 pixel, so that stars appear where expected....I'm
...
;contour frame #20
t=20
im=smooth(images(*,*,t),3)
window, xs=Nx, ys=Ny, retain=2
set_plot,'ps'
device,xsize=16,ysize=21,xoffset=3,yoffset=3.5, /portrait,bits_per_pixel=8
contour, alog(im>5.0), nlevels=13,xstyle=1,ystyle=1, $
  title='Image #'+strtrim(string(t),2)+', Orbit '+strtrim(string(orbit),2),charsize=1.3, $
  xtitle='pixels',ytitle='pixels'
output_plot, '20orbit193.ps'
...
sun_pix_xy=array_indices(im, sun_pix)
x_sun = sun_pix_xy[0]
y_sun = sun_pix_xy[1]
im(sun_pix)=255
window,xs=Nx,ys=Ny
...
im0 = images[*,*,0]
im1 = images[*,*,35]
im2x2 = fltarr(2*Nx, 2*Ny)
mx = max([im0, im1])
mn = min([im0, im1])
im0[x_sun-5:x_sun+5, y_sun] = mx
im0[x_sun, y_sun-5:y_sun+5] = mx
im1[x_sun-6:x_sun+6, y_sun] = mx
im1[x_sun, y_sun-6:y_sun+6] = mx
threshold=70.0
im2x2[0,Ny] = im0
im2x2[Nx,Ny]= im1
im2x2[0,0] = (im0>0<threshold)*(mx/threshold)
im2x2[Nx,0]= (im1>0<threshold)*(mx/threshold)
im2x2 = fix(255.0*(im2x2-mn)/(mx-mn))
window, xs=2*Nx, ys=2*Ny, retain=2
tv, im2x2
write_tiff, 'orbit193_2x2.tiff', transpose(rotate(im2x2,1))
...
y=487 & plot,avg_image(*,488)<40,thick=3 & oplot,avg_image(*,y),psym=1 & 
    print,max(avg_image(*,y))
x=257 & plot,avg_image(257,*),xrange=[400,550],thick=3 & oplot,avg_image(x,*)

;store results
save, filename = 'LHG193.dat', images, exp_time, phi, theta, date, target_distance, $
    mask, altitude, sc_x, sc_y, sc_z
